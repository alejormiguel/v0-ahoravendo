...
export function PostalCodeModal({ open, onOpenChange, onSelectPostalCode }: PostalCodeModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg py-12">
        <DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            >
              <Icon icon="agregar-circulo" size={16} color="filter-fucsia-svg" /> Crear nueva
            </Button>
          </div>
          ...
          <div>
            ...
            <Input
              ...
              className="mb-2"
            />
            <a href="#" className="text-fuchsia-600 underline text-sm">
              No sé mi código
            </a>
          </div>
        </div>

        <DialogFooter className="flex justify-center sm:justify-center mt-6 border-t pt-4 gap-4">
          <Button
            variant="outline"
            onClick={() => onOpenChange(false)}
            className="border-fuchsia-600 text-fuchsia-600 hover:bg-fuchsia-50 rounded-xl px-6"
          >
            Cancelar
          </Button>
          <Button onClick={handleSubmit} className="bg-gray-200 text-gray-700 hover:bg-gray-300 rounded-xl px-6" disabled={!postalCode}>
            Confirmar
          </Button>
        </DialogFooter>
      </DialogContent>
